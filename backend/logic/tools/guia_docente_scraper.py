...
class UGRTeachingGuideScraper:
            ...
            for prof_div in profesor_divs:
                ...
                for header in nombre_headers:
                    if self._clean_text(header.get_text()) in nombre:
                        ...
                        if tutorias_section:
                            tutorias_text = self._clean_text(tutorias_section.get_text())
                            break
                
                if nombre and nombre not in [p['nombre'] for p in profesores]:
                    profesores.append({
                        "nombre": nombre,
                        "tutorias": tutorias_text
                    })
                    ...
                    for h3 in h3_headers:
                        ...
                        if content_div:
                            ...
                            if 'teórico' in h3_text.lower():
                                self.data['programa_de_contenidos_teóricos_y_prácticos']['teórico'] = content
                            elif 'práctico' in h3_text.lower():
                                self.data['programa_de_contenidos_teóricos_y_prácticos']['práctico'] = content
                    ...
                    for h3 in h3_headers:
                        ...
                        if content_div:
                            ...
                            if 'fundamental' in h3_text.lower():
                                self.data['bibliografía']['bibliografía_fundamental'] = biblio_items
                            elif 'complementaria' in h3_text.lower():
                                self.data['bibliografía']['bibliografía_complementaria'] = biblio_items
                    ...
                    for h3 in h3_headers:
                        ...
                        if content_div:
                            ...
                            if 'ordinaria' in h3_text.lower():
                                self.data['evaluación']['evaluación_ordinaria'] = content
                            elif 'extraordinaria' in h3_text.lower():
                                self.data['evaluación']['evaluación_extraordinaria'] = content
                            elif 'única final' in h3_text.lower():
                                self.data['evaluación']['evaluación_única_final'] = content
    ...
    def parse(self) -> Dict[str, Any]:
        ...
        self.data['prerrequisitos_o_recomendaciones'] = self._extract_section_content('Prerrequisitos')
        self.data['breve_descripción_de_contenidos'] = self._extract_section_content('Breve descripción')
        self.data['resultados_de_aprendizaje'] = self._extract_section_content('Resultados de aprendizaje')
        self.data['enlaces_recomendados'] = self._extract_section_content('Enlaces recomendados')
        self.data['metodología_docente'] = self._extract_section_content('Metodología docente')
...
def main():
    ...
    # Leer archivo HTML
    with open('guia_docente_tfg.html', 'r', encoding='utf-8') as f:
        html_content = f.read()
