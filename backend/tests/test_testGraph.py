...
def test_initialize_test_creates_questions_and_state(
	monkeypatch, dummy_multiple_choice_test, dummy_tool_factory, patch_get_tools, dummy_llm_factory, dummy_message_factory
):
	...
	monkeypatch.setattr(tg.TestSessionGraph, "_get_llm", lambda self, temp=None: dummy_llm_factory())

	tsg = tg.TestSessionGraph()

	# Simulate last message having tool_calls with args
	last_msg = dummy_message_factory(tool_calls=[{"args": {"topic": "math", "num_questions": 2}}])
	state = {"messages": [last_msg]}

	init_state = tsg.initialize_test(state)
...
def test_present_question_returns_ai_message_content(monkeypatch, dummy_multiple_choice_test, dummy_llm_factory):
	import backend.logic.testGraph as tg
	...
	state = {"questions": [question], "current_question_index": 0}

	out = tsg.present_question(state)
...
def test_answer_question_interrupt_and_evaluation(
	monkeypatch, dummy_multiple_choice_test, dummy_llm_factory, patch_get_tools, interrupt_simulator
):
	import backend.logic.testGraph as tg

	# Patch LLM to return a clear CORRECT and FEEDBACK response
	monkeypatch.setattr(tg.TestSessionGraph, "_get_llm", lambda self, temp=None: dummy_llm_factory("CORRECT: YES\nFEEDBACK: Bien hecho"))
	...
	assert any(isinstance(m, object) and getattr(m, "content", "") for m in out["messages"])
...
def test_test_router_continues_and_finalizes():
	...
	# continue case
	st = {"current_question_index": 0, "num_questions": 2}
	assert tsg.test_router(st) == "continue"

	# finalize case
	st = {"current_question_index": 2, "num_questions": 2}
	assert tsg.test_router(st) == "finalize"


def test_finalize_test_returns_toolmessage_with_tool_call_id(monkeypatch, dummy_llm_factory, dummy_message_factory):
	import backend.logic.testGraph as tg
...
def test_evaluate_answer_with_llm_parsing(monkeypatch, dummy_llm_factory, dummy_multiple_choice_test):
	import backend.logic.testGraph as tg
	...
	MCQ = dummy_multiple_choice_test
	q = MCQ("¿2+2?", ["4", "3"])
	state = {"topic": "matematicas"}

	feedback, is_correct = tsg.evaluate_answer_with_llm(q, "4", state)
