...
class HealthCheckResponse(BaseModel):
    ...
    status: str = Field(
        ..., description="Health status of the API, e.g., 'healthy' or 'unhealthy'"
    )
    qdrant_connected: bool = Field(
        ..., description="Whether Qdrant vector store is connected"
    )
    collection: dict | None = Field(
        None, description="Collection information from vector store"
    )
    message: str | None = Field(
        None, description="Additional message about the health status"
    )
...
class DocumentMetadata(BaseModel):
    ...
    filename: str | None = Field(None, description="Original file name of the document")
    ...
    fuente: str = Field(
        default="PRADO UGR", description="Source: 'PRADO UGR', 'Wikipedia', etc."
    )
    idioma: str = Field(default="es", description="Language code: 'es' or 'en'")
    chunk_id: int | None = Field(None, description="Chunk ID within the document")
    licencia: str | None = Field(
        None, description="Document license, e.g., 'CC-BY', 'MIT', etc."
    )
...
class QueryRequest(BaseModel):
    ...
    asignatura: str | None = Field(None, description="Filter by subject")
    tipo_documento: str | None = Field(None, description="Filter by document type")
...
class IndexResponse(BaseModel):
    ...
    timestamp: datetime = Field(
        default_factory=datetime.now, description="Indexing timestamp"
    )
...
class LoadFileResponse(BaseModel):
    ...
    timestamp: datetime = Field(
        default_factory=datetime.now, description="Load timestamp"
    )
...
class UploadFileMetadata(DocumentMetadata):
    ...
    auto_index: bool = Field(
        default=True, description="Automatically index after upload"
    )
