...
def test_list_subjects_route(api_client, monkeypatch):
    ...
    monkeypatch.setattr(subjects_module, "ls_subjects", lambda: ["matematica", "fisica"])
...
def test_list_document_types_route(api_client, monkeypatch):
    monkeypatch.setattr(subjects_module, "ls_document_types", lambda asignatura: ["apuntes", "examen"])
