...
def upload_document(
    ...
    if file_path_obj.suffix.lower() not in valid_extensions:
        print(f"❌ Error: Extensión no soportada: {file_path_obj.suffix}")
    ...
    print(f"{'=' * 60}")
    print(f"Archivo:     {file_path_obj.name}")
    print(f"Tamaño:      {file_path_obj.stat().st_size / 1024:.2f} KB")
    ...
    try:
        files = {"file": open(file_path_obj, "rb")}
        print()
...
def list_files(asignatura: str | None = None, tipo_documento: str | None = None):
    """
    ...
    try:
        ...
        print("❌ ERROR: No se puede conectar con el servicio RAG\n")
...
def search_documents(query: str, asignatura: str | None = None, top_k: int = 3):
    """
...
def check_health():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"Qdrant:         {'✅ Conectado' if data['qdrant_connected'] else '❌ Desconectado'}"
            )
    ...
    if len(sys.argv) == 1:
        ...
        if opcion == "1":
            ...
            tipo_documento = input(
                "Tipo (apuntes/ejercicios/examenes/practicas): "
            ).strip()
            ...
            upload_document(
                file_path, asignatura, tipo_documento, tema, autor, licencia=licencia
            )
        elif opcion == "2":
            asignatura_input = input(
                "Filtrar por asignatura (Enter para todas): "
            ).strip()
            asignatura_filter: str | None = (
                asignatura_input if asignatura_input else None
            )
            list_files(asignatura_filter)
        elif opcion == "3":
            list_subjects()
        elif opcion == "4":
            query = input("Búsqueda: ").strip()
            asignatura_search_input = input(
                "Filtrar por asignatura (Enter para todas): "
            ).strip()
            asignatura_search: str | None = (
                asignatura_search_input if asignatura_search_input else None
            )
            search_documents(query, asignatura_search)
        else:
            print("Saliendo...\n")
    # Modo con argumentos
    elif len(sys.argv) >= 4:
        ...
        print("\n📖 USO:")
        print(
            f"  {sys.argv[0]} <archivo> <asignatura> <tipo> [tema] [autor] [licencia]"
        )
        print("\nEJEMPLOS:")
        print(
            f'  {sys.argv[0]} apuntes.pdf "Lógica Difusa" apuntes "Conjuntos difusos" "CC-BY-4.0"'
        )
