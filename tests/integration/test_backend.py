...
def test_chat_endpoint_basic_conversation(api_base_url, session_id, api_timeout):
    ...
    payload = {"query": "Hola, ¿cómo estás?", "id": session_id}
...
def test_chat_endpoint_with_tools(api_base_url, session_id, api_timeout):
    ...
    payload = {"query": "¿Cuánto es 2 + 2?", "id": session_id}
...
def test_chat_endpoint_with_memory(api_base_url, session_id, api_timeout):
    ...
    # Primera interacción
    payload_1 = {"query": "Mi nombre es Alicia", "id": session_id}
    response_1 = requests.post(
        f"{api_base_url}/chat", json=payload_1, timeout=api_timeout
    )
    ...
    payload_2 = {"query": "¿Cuál es mi nombre?", "id": session_id}
    response_2 = requests.post(
        f"{api_base_url}/chat", json=payload_2, timeout=api_timeout
    )
...
def test_chat_endpoint_invalid_payload(api_base_url, api_timeout):
    ...
    response = requests.post(
        f"{api_base_url}/chat", json=invalid_payload, timeout=api_timeout
    )
...
def test_chat_endpoint_different_sessions(api_base_url, session_id, api_timeout):
    ...
    payload_1 = {"query": "Mi nombre es Roberto", "id": session_id}
    requests.post(f"{api_base_url}/chat", json=payload_1, timeout=api_timeout)

    # Segunda sesión - establecer nombre diferente
    payload_2 = {"query": "Mi nombre es Carolina", "id": session_id_2}
    requests.post(f"{api_base_url}/chat", json=payload_2, timeout=api_timeout)

    # Verificar primera sesión
    payload_verify_1 = {"query": "¿Cuál es mi nombre?", "id": session_id}
    response_1 = requests.post(
        f"{api_base_url}/chat", json=payload_verify_1, timeout=api_timeout
    )
    result_1 = response_1.json()
    assert "Roberto" in result_1["messages"][-1]["content"]

    # Verificar segunda sesión
    payload_verify_2 = {"query": "¿Cuál es mi nombre?", "id": session_id_2}
    response_2 = requests.post(
        f"{api_base_url}/chat", json=payload_verify_2, timeout=api_timeout
    )
    result_2 = response_2.json()
