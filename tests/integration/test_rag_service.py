...
def test_rag_index_and_search_workflow(rag_base_url, api_timeout):
    ...
    index_response = requests.post(
        f"{rag_base_url}/index", json=documents, timeout=api_timeout
    )
    ...
    search_response = requests.post(
        f"{rag_base_url}/search", json=search_payload, timeout=api_timeout
    )
...
def test_rag_list_document_types(rag_base_url, api_timeout):
    ...
    response = requests.get(
        f"{rag_base_url}/subjects/TestSubject/types", timeout=api_timeout
    )
    assert response.status_code == 200
...
def test_rag_upload_text_file(rag_base_url, api_timeout):
    ...
    files = {"file": ("test_ai.txt", file_data, "text/plain")}
    ...
    response = requests.post(
        f"{rag_base_url}/upload", files=files, data=data, timeout=api_timeout
    )
...
def test_rag_upload_markdown_file(rag_base_url, api_timeout):
    ...
    file_content = b"""# Machine Learning
...
"""
    ...
    files = {"file": ("test_ml.md", file_data, "text/markdown")}
...
def test_rag_list_files_with_filters(rag_base_url, api_timeout):
    ...
    params = {"asignatura": "IA", "tipo_documento": "apuntes"}

    response = requests.get(f"{rag_base_url}/files", params=params, timeout=api_timeout)
    assert response.status_code == 200
...
def test_rag_index_empty_documents(rag_base_url, api_timeout):
    ...
    response = requests.post(
        f"{rag_base_url}/index", json=index_payload, timeout=api_timeout
    )
    assert response.status_code == 200
...
def test_rag_invalid_search_payload(rag_base_url, api_timeout):
    ...
    response = requests.post(
        f"{rag_base_url}/search", json=invalid_payload, timeout=api_timeout
    )
...
def test_rag_invalid_index_payload(rag_base_url, api_timeout):
    ...
    response = requests.post(
        f"{rag_base_url}/index", json=invalid_payload, timeout=api_timeout
    )
    assert response.status_code == 422  # Unprocessable Entity
...
def test_rag_upload_file_without_auto_index(rag_base_url, api_timeout):
    ...
    file_content = b"Documento sin indexar automaticamente."
    file_data = BytesIO(file_content)
    file_data.name = "test_no_index.txt"

    files = {"file": ("test_no_index.txt", file_data, "text/plain")}
    ...
    assert (
        "indexed" not in result.get("message", "").lower()
        or "not" in result.get("message", "").lower()
    )
...
def test_rag_search_with_multiple_filters(rag_base_url, api_timeout):
    ...
    search_payload = {
        "query": "matemáticas",
        "top_k": 5,
        "filters": {"asignatura": "Matemáticas", "tipo_documento": "examen"},
    }
